...
export type UpdateSportsNewsSettingsInput = {
  ...
  sameDaySourcesOnly?: boolean; maxSourceAgeHours?: number; requirePublishedAt?: boolean;
  requireSourceUrl?: boolean; minimumSources?: number; freshnessFallbackEnabled?: boolean;
...
export class SportsNewsSettingsService {
  ...
  async get() {
    ...
    return this.prisma.sportsNewsSetting.upsert({ where: { workspaceId: workspace.id }, update: {}, create: { workspaceId: workspace.id }, include: { telegramChannel: true, facebookChannel: true } });
  }
  async update(input: UpdateSportsNewsSettingsInput) {
    ...
    if (input.maxSourceAgeHours !== undefined && (input.maxSourceAgeHours < 1 || input.maxSourceAgeHours > 168)) throw new BadRequestException('maxSourceAgeHours must be between 1 and 168.');
    if (input.minimumSources !== undefined && (input.minimumSources < 1 || input.minimumSources > 20)) throw new BadRequestException('minimumSources must be between 1 and 20.');
  ...
  async channels() {
    const workspace = await this.workspace();
    return this.prisma.socialChannel.findMany({ where: { workspaceId: workspace.id }, select: { id: true, platform: true, name: true, username: true, externalId: true, status: true, lastError: true }, orderBy: [{ platform: 'asc' }, { name: 'asc' }] });
  }
  async markRun(kind: 'morning' | 'evening', status: string, error?: string | null) {
    const settings = await this.get();
    return this.prisma.sportsNewsSetting.update({ where: { id: settings.id }, data: { ...(kind === 'morning' ? { lastMorningRunAt: new Date() } : { lastEveningRunAt: new Date() }), lastRunStatus: status, lastError: error ?? null } });
  }
