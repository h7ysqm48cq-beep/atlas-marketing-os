...
export class SportsNewsRunnerService {
  ...
  constructor(private readonly settingsService: SportsNewsSettingsService, private readonly sourceService: SportsNewsSourceService, private readonly sourceValidator: SportsNewsSourceValidatorService, private readonly generator: SportsNewsGeneratorService) {}
  ...
  async run(kind: SportsNewsRunKind, trigger: 'schedule' | 'manual' = 'schedule') {
    ...
    try {
      ...
      await this.settingsService.markRun(kind, 'CONTENT_READY');
      this.logger.log(`Sports news ${kind}: content ready from ${validation.accepted.length} verified source(s). Model=${generated.model}`);
      return { skipped: false, kind, trigger, status: 'CONTENT_READY', provider: sourceResult.provider, fetchedAt: sourceResult.fetchedAt, sourceCount: validation.accepted.length, rejectedSourceCount: validation.rejected.length, content: generated.content, ai: { provider: generated.provider, model: generated.model, usage: generated.usage }, destinations: { telegram, facebook } };
    } catch (error) {
