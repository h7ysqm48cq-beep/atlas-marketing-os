...
export class SportsNewsRunnerService {
  ...
  async run(kind: SportsNewsRunKind, trigger: 'schedule' | 'manual' = 'schedule') {
    ...
    try {
      ...
      const lock = await this.runHistory.acquire({ kind, trigger, timezone: settings.timezone });
      if (!lock.acquired) {
        this.logger.warn(`Sports news ${kind}: duplicate scheduled run skipped (${lock.runKey}).`);
        return { skipped: true, reason: 'duplicate_scheduled_run', kind, runKey: lock.runKey };
      }
      runId = lock.run.id;
      ...
      const validation = this.sourceValidator.validate(sourceResult.sources, { timezone: settings.timezone, sameDaySourcesOnly: settings.sameDaySourcesOnly, maxSourceAgeHours: settings.maxSourceAgeHours, requirePublishedAt: settings.requirePublishedAt, requireSourceUrl: settings.requireSourceUrl, minimumSources: settings.minimumSources, freshnessFallbackEnabled: settings.freshnessFallbackEnabled });
      ...
      const generated = await this.generator.generate(kind, validation.accepted, { language: settings.language, sportsKnowledgeEnabled: settings.sportsKnowledgeEnabled, discussionQuestionEnabled: settings.discussionQuestionEnabled, referenceLinksEnabled: settings.referenceLinksEnabled, customPromptEnabled: settings.customPromptEnabled, systemPrompt: settings.systemPrompt, morningPrompt: settings.morningPrompt, eveningPrompt: settings.eveningPrompt, knowledgePrompt: settings.knowledgePrompt, customInstructions: settings.customInstructions });
      ...
      if (settings.imageEnabled) {
        ...
        image = await this.imageGenerator.generate(kind, generated.content, { imageEnabled: settings.imageEnabled, imagePrompt: settings.imagePrompt, morningImagePrompt: settings.morningImagePrompt, eveningImagePrompt: settings.eveningImagePrompt, imageAspectRatio: settings.imageAspectRatio, imageTextMode: settings.imageTextMode, imageVisualStyle: settings.imageVisualStyle, logoEnabled: settings.logoEnabled, logoPosition: settings.logoPosition, brandFooterEnabled: settings.brandFooterEnabled, brandFooterText: settings.brandFooterText });
      }

      await this.settingsService.markRun(kind, 'QUEUEING');
      const queued = await this.newsPublisher.queue({ kind, content: generated.content, mediaUrls: image?.dataUrl ? [image.dataUrl] : [], timezone: settings.timezone, autoPublishEnabled: settings.autoPublishEnabled, approvalRequired: settings.approvalRequired, telegram, telegramChannelId: settings.telegramChannelId, facebook, facebookChannelId: settings.facebookChannelId });
      ...
      await this.runHistory.complete(runId, { status: finalStatus, sourceCount, rejectedSourceCount, scheduledPostIds: postIds });
      this.logger.log(`Sports news ${kind}: ${finalStatus}. ${sourceCount} verified source(s), ${queued.posts.length} post(s), image=${Boolean(image)}.`);
      return { skipped: false, kind, trigger, runId, status: finalStatus, provider: sourceResult.provider, fetchedAt: sourceResult.fetchedAt, sourceCount, rejectedSourceCount, content: generated.content, imageGenerated: Boolean(image), scheduledPostIds: postIds, scheduledPostStatus: queued.status, ai: { provider: generated.provider, model: generated.model, usage: generated.usage }, destinations: { telegram, facebook } };
    } catch (error) {
      ...
      if (runId) await this.runHistory.complete(runId, { status: 'FAILED', sourceCount, rejectedSourceCount, error: message }).catch(() => undefined);
