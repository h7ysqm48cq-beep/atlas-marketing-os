...
import {
  SportsNewsRunnerService,
  SportsNewsRunKind,
} from './sports-news-runner.service';
...
export class AutomationSchedulerService {
  ...
  async publishDuePosts() {
    ...
    try {
      ...
      if (result.found > 0)
        this.logger.log(
          `Publisher cycle completed. Found: ${result.found}. Published: ${result.published}. Duration: ${Date.now() - startedAt}ms.`,
        );
    } catch (error: unknown) {
      const message =
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : 'Unknown sports news scheduler error';
  ...
  private async runIfDue(
    ...
    try {
      await this.sportsNewsRunner.run(kind, 'schedule');
    } catch {
      this.lastSportsNewsSlot.delete(kind);
    }
  }
