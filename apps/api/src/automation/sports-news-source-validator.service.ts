...
export type SportsNewsFreshnessRules = {
  timezone: string;
...
export class SportsNewsSourceValidatorService {
  validate(
    ...
    rules: SportsNewsFreshnessRules,
    now = new Date(),
  ) {
    const accepted: SportsNewsSource[] = [];
    ...
    const minimumSources = Math.max(1, rules.minimumSources);
    ...
    return {
      ...
      rejected,
      enoughSources,
      fallbackAllowed: !enoughSources && rules.freshnessFallbackEnabled,
  ...
  private rejectionReason(
    ...
    if (rules.requireSourceUrl && !source.url?.trim()) {
      return 'missing_source_url';
    }

    if (rules.requirePublishedAt && !source.publishedAt) {
    ...
    if (ageHours > rules.maxSourceAgeHours) {
      return 'source_too_old';
    }

    if (
      rules.sameDaySourcesOnly &&
      this.localDate(published, rules.timezone) !==
        this.localDate(now, rules.timezone)
    ) {
      return 'not_same_local_day';
    }
