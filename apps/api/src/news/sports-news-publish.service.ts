...
export class SportsNewsPublishService {
  ...
  async queue(input: {
    ...
    const channelIds = [
      input.telegram ? input.telegramChannelId : null,
      input.facebook ? input.facebookChannelId : null,
    ...
    const status =
      input.approvalRequired || !input.autoPublishEnabled
        ? ScheduledPostStatus.DRAFT
        : ScheduledPostStatus.QUEUED;
    const now = new Date();
    const created: Awaited<
      ReturnType<typeof this.prisma.scheduledPost.create>
    >[] = [];
    const add = async (channelId: string | null, platform: SocialPlatform) => {
      ...
      if (channel.platform !== platform)
        throw new Error(`Selected channel is not ${platform}.`);
      created.push(
    ...
    if (input.telegram)
      await add(input.telegramChannelId, SocialPlatform.TELEGRAM);
    if (input.facebook)
      await add(input.facebookChannelId, SocialPlatform.FACEBOOK);
    return {
      posts: created,
      status,
      queued: status === ScheduledPostStatus.QUEUED,
    };
  }
}
