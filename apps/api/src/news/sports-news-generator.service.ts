...
export class SportsNewsGeneratorService {
  ...
  async generate(
    ...
    settings: SportsNewsGenerationSettings,
  ) {
    if (!sources.length)
      throw new Error('Cannot generate sports news without verified sources.');
    const sourceBlock = sources
      ...
      .join('\n\n');
    const defaultSystem = `You are the M-Sports / 满贯门体育 newsroom editor for a Malaysia audience. Use ONLY the verified source list supplied by the user. Never invent scores, fixtures, quotes, injuries, transfers, dates, rankings, source URLs or events. If a fact is not supported by the supplied sources, omit it. Keep news and sports knowledge clearly separated. Output polished publish-ready copy.`;
    const editionPrompt =
      kind === 'morning' ? settings.morningPrompt : settings.eveningPrompt;
    const language =
