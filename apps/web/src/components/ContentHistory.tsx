...
export function ContentHistory() {
  ...
  async function load(){
    try{ const response=await fetch(`${API_BASE_URL}/history`,{cache:"no-store"}); const data=await response.json() as HistoryRecord[]|{message?:string};
      ...
      setRecords(data); setSelected((current)=>data.find((item)=>item.id===current?.id)||data[0]||null); setStatus(data.length?`${data.length} saved generations.`:"No saved generations yet.");
  ...
  return <div className={styles.page}>
    ...
    <div className={styles.viewer}>{!selected?<div className={styles.emptyViewer}>Select a generation to inspect it.</div>:<><div className={styles.viewerHeader}><div><div className={styles.viewerBadges}><span className={styles.brandBadge}>{selected.brand.name}</span><span className={`${styles.statusBadge} ${styles[`status${selected.status}`]}`}>{formatStatus(selected.status)}</span></div><h2>{selected.topic}</h2><p>{selected.analysis.summary||"Saved AI generation"}</p></div><div className={styles.actions}><button onClick={()=>void toggleFavorite(selected)}>{selected.isFavorite?"★ Favorited":"☆ Favorite"}</button><button onClick={()=>void copySelected()}>Copy</button><button className={styles.deleteButton} onClick={()=>void remove(selected)}>Delete</button></div></div>
