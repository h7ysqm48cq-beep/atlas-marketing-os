...
const EXPECTED_PARENT = 'e98eb9af7a5be25d2f8f871d29dd171a29fe14e2';
const ALLOWED_FILES = new Set([
  'apps/engineering-runner/railway.json',
  'apps/engineering-runner/check-runner-bootstrap-deployment.cjs',
]);

function requireExact(env, key, expected) {
  if (env[key] !== expected) {
    throw new Error('ATLAS_RUNNER_BOOTSTRAP_DENY ' + key);
  }
}
...
async function main(env = process.env, fetchImpl = globalThis.fetch) {
  ...
  if (!/^[0-9a-f]{40}$/.test(sha || '')) {
    throw new Error('ATLAS_RUNNER_BOOTSTRAP_DENY commit');
  }
  if (typeof fetchImpl !== 'function') {
    throw new Error('ATLAS_RUNNER_BOOTSTRAP_DENY fetch');
  }
  const url = 'https://api.github.com/repos/' + OWNER + '/' + REPO + '/commits/' + sha;
  const response = await fetchImpl(url, {
    headers: {
      accept: 'application/vnd.github+json',
      'user-agent': 'atlas-runner-bootstrap-check',
    },
    signal: AbortSignal.timeout(10000),
  });
  if (!response.ok) {
    throw new Error('ATLAS_RUNNER_BOOTSTRAP_DENY github_' + response.status);
  }
  const commit = await response.json();
  if (commit.sha !== sha) throw new Error('ATLAS_RUNNER_BOOTSTRAP_DENY sha');
  if (!Array.isArray(commit.parents) ||
      commit.parents.length < 1 ||
      (commit.parents[0] || {}).sha !== EXPECTED_PARENT) {
    throw new Error('ATLAS_RUNNER_BOOTSTRAP_DENY parent');
  }
  ...
  if (files.length !== ALLOWED_FILES.size ||
    ...
    throw new Error('ATLAS_RUNNER_BOOTSTRAP_DENY scope');
  }
