...
- A Human Owner production deployment authorization is not itself permission to call Railway. Before any production redeploy mutation, the CI/external deployment orchestrator must atomically claim the exact approved task/execution/service/provenance through `production-deployment/claim-mutation`; the claim consumes that Owner authorization before Railway is touched.
- One claimed Human Owner deployment authorization permits at most one Railway mutation attempt. The caller must never auto-retry, tool-retry or manually repeat the mutation after a timeout, connection reset, 5xx, malformed response or other ambiguous outcome.
- After an ambiguous production mutation outcome, reconcile read-only using persisted Supervisor state plus Railway deployment/list/status evidence. If the outcome cannot be proven, keep the authorization spent and require a fresh Human Owner authorization for any new mutation attempt; never reopen or recycle the previous claim.
- Railway `preDeploy` resolution must require the exact pre-mutation claim for the resolved task/execution/service. Missing or mismatched claim state is fail-closed. Never bypass the claim by calling Railway directly through another agent, connector, CLI or API path.
