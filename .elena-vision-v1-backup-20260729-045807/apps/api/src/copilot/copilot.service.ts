...
export class CopilotService {
  ...
  async chat(dto: ChatCopilotDto) {
    ...
    const attachments = this.cleanAttachments(
      dto.attachments,
    );
    ...
    const [
      ...
      this.getAttachmentDocumentContext(
        attachments,
        brand.id,
      ),
    ]);
...
Description: ${campaign.description || 'Not set'}`
  ...
  private cleanAttachments(
    attachments: ChatCopilotDto['attachments'],
  ) {
