    ...
    try {
      for (const file of files) {
        ...
        if (file.type.startsWith('image/')) {
          ...
          const response = await fetch(
            `${API_URL}/copilot/attachments/image`,
            {
              method: 'POST',
              body: formData,
            },
        ...
        if (!documentSupported) {
          throw new Error(
            `${file.name} is not a supported attachment.`,
          );
        }
        ...
        setAttachments((current) => [
          ...current,
          {
            ...
            url,
            storageProvider:
              data.upload?.storageProvider,
            storagePath:
              data.upload?.storagePath,
            documentId:
              data.document?.id,
          },
        ]);
      ...
      setStatus(
        'Attachment ready. Add your instruction and send.',
      );
    } catch (error) {
      setStatus(
        error instanceof Error
                ...
                {attachments.map((attachment) => (
                  ...
                  >
                    {attachment.kind === 'image' ? (
                      ...
                      <div
                        className={styles.documentAttachmentIcon}
                      >
                        DOC
                      </div>
                    )}
